...
const ProjectCard = ({ project, count }: ProjectCardProps) => {
  return (
    <a href={project.link} className="transform-[rotateX(180deg)] h-auto relative group w-85 md:h-100 md:w-130 lg:h-150 lg:w-200 shrink-0 border border-pink-200/30 mr-5 flex flex-col px-10 py-10 rounded-3xl">
      ...
      <div className="flex justify-between items-center mb-5">
        <span className="material-symbols-outlined text-pink-200 animate-jump">
          left_click
        </span>
        <h1 className="saira-condensed-light z-1 text-gray-200/60">
          {count < 10 ? "0" : ""}
          {count}
        </h1>
      </div>
      ...
      <div className="md:h-full h-auto z-10 flex flex-col md:justify-end ">
        ...
        <p className="overflow-y-scroll text-[.9rem] lg:text-[1rem] text-white/50 lg:opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          {project.description}
        </p>
      </div>
    </a>
